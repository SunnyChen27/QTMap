...
void World::Monstermove(int direction, int steps){
    if(movejustify(direction, steps, this->_monster.getPosX(), this->_monster.getPosY() + 1, 0))
...
void World::setMonster(char *line){
    char ox[3], oy[3];
    setxsety(line, ox, oy, 8);

    this->_monster.initObj("monster");
    this->_monster.setPosX(atoi(ox));
    this->_monster.setPosY(atoi(oy));
}
...
void World::savedata(){
    ofstream savefile("D:\\DLJ\\study\\programming\\QTMap\\savedata.txt");
    savefile << "player," << this->_player.getPosX() << '-' << this->_player.getPosY() << ",\n";
    savefile << "monster," << this->_monster.getPosX() << '-' << this->_monster.getPosY() << ",\n";
